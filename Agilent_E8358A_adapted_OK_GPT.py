...
# ------------------ low level helpers (from script 1) ------------------
...
def recv_exact(sock: socket.socket, n: int) -> bytes:
    ...
    while len(data) < n:
        ...
        if not chunk:
            raise ConnectionError("Socket closed while recv_exact")
        data += chunk
    return data
...
# ------------------ session management using raw socket ------------------

def open_session(ip: str, timeout_ms: int = 8000) -> Sess:
    """Open a plain TCP socket to the instrument (SCPI over TCP)."""
    ...
    try:
        ...
        s.close()
        raise RuntimeError(f"Could not open {ip}:{TCP_PORT}: {e}")
    # small pause like in your first script
    time.sleep(0.05)
    return Sess(sock=s, address=ip)


def close_session(sess: Optional[Sess]) -> None:
    if not sess:
        return
    try:
...
# ------------------ high level SCPI helpers (adapted to socket) ------------------

def write(s: Sess, cmd: str) -> None:
    if not cmd.endswith("\n"):
        cmd = cmd + "\n"
    log_print(f">> {cmd.strip()}")
    try:
        s.sock.sendall(cmd.encode())
    except Exception as e:
...
def query(s: Sess, cmd: str, timeout_ms: Optional[int] = None) -> str:
    ...
    if timeout_ms is not None:
        s.sock.settimeout(timeout_ms / 1000.0)
    try:
        write(s, cmd)
        resp = recv_line(s.sock)
        log_print(f"?? {resp}")
        return resp.strip()
    finally:
...
def configure_sweep(s: Sess, center_hz: float, span_hz: float, points: int, ifbw_hz: float) -> None:
    ...
    # Data format - use binary REAL 32 like in your working driver for speed/reliability
    write(s, "FORM REAL,32")
    write(s, "FORM:BORD NORM")
...
def run_sweep(s: Sess, timeout_s: float = 120.0) -> bool:
    ...
    try:
        resp = query(s, "*OPC?", timeout_ms=int(timeout_s * 1000))
        return resp.strip() == "1"
...
def fetch_data(s: Sess) -> Tuple[List[float], List[float], List[float]]:
    """
    Fetch frequency axis and complex SDATA.
    Frequencies: ASCII CSV from SENS#:FREQ:DATA?
    SDATA: binary block returned by CALC#:DATA? SDATA (REAL32 pairs)
    """
    # frequencies (ASCII)
    f_csv = query(s, f"SENS{CH}:FREQ:DATA?")
    freqs = [float(v) for v in f_csv.split(",") if v]

    # request SDATA as binary block
    write(s, f"CALC{CH}:DATA? SDATA")
    raw = read_binary_block(s.sock, dtype=">f4")  # big-endian float32 pairs [Re, Im, Re, Im, ...]
    if raw.size % 2 != 0:
        # try to be forgiving: trim last element
        raw = raw[:-1]
    reals = raw[0::2].tolist()
    imags = raw[1::2].tolist()
...
class Main(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PNA minimal (quiet S11/S21) - socket backend")
        self.resize(740, 420)
